...
**Trade-offs and challenges:** The two-pass design adds latency (two sequential LLM calls instead of one). The summarization call receives up to 200k characters of code plus tree and README, which can take several seconds even with `gpt-5-mini`'s speed. We rely entirely on the first LLM's judgment for file selection; if it picks poorly (ignoring key modules or choosing noise), the summary suffers. The blocklist and budget caps mitigate the worst cases but don't guarantee optimal choice. An alternative would be single-pass with static heuristics (faster, more predictable) or multi-turn agentic exploration (flexible but complex).
